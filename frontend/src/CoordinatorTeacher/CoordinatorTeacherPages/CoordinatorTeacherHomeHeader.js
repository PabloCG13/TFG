...
const CoordinatorTeacherHomeHeader = ({ teacherId, logout }) => {
  ...
  const fetchValidations = () => {
    ...
    fetch(`http://localhost:5000/api/degrees/${teacherId}`)
      ...
      .then((data) => {
        ...
        return Promise.all([
          fetch(
            `http://localhost:5000/api/validations/provisional/answers/${data.unicode}/${data.degreeid}`
          ).then((res) => res.json()),
          fetch(
            `http://localhost:5000/api/validations/provisional/notification/${data.unicode}/${data.degreeid}`
          ).then((res) => res.json()),
          fetch(
            `http://localhost:5000/api/transcripts/notification/erasmus/${data.unicode}/${data.degreeid}`
          ).then((res) => res.json()),
        ]);
      })
  ...
  //Method to update the lastAccessDate of the Degree Coordinator
            ...
            >
              ...
              {(validations.length > 0 ||
                ...
                <span style={notificationBadgeStyle}>
                  {validations.length +
                    newValidations.length +
                    erasmusGradeConfirmations.length}
                </span>
              )}
            </button>
          </div>
            ...
            erasmusGradeConfirmations.length > 0 ? (
              <ul>
                {validations.map((validation, index) => (
                  <li key={index}>
                    There is an answer for the validation request for the course{" "}
                ...
                {newValidations.map((validation, index) => (
                  <li key={index}>
                    There is a new request validation for the course{" "}
                    <strong>{validation.courseidsrc}</strong>
                    <Link
                      to={`/CoordinatorTeacher/CoordinatorTeacherPages/CoordinatorTeacherConfirmValidationPage/CoordinatorTeacherConfirmValidation/${teacherId}`} // Route where it links to
                      state={{ participantAddress }} // Route where it links to
                ...
                {erasmusGradeConfirmations.map((confirmation, index) => (
                  <li key={index}>
                    Student <strong>{confirmation.studentid}</strong> has been
                    graded by Erasmus teacher {confirmation.teacherid} on
                    subject {confirmation.courseid}
                    <Link
