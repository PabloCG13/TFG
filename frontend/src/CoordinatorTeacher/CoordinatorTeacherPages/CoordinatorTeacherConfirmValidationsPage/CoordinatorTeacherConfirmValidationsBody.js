...
const CoordinatorTeacherConfirmValidationBody = ({ teacherId }) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:5000/api/validations/provisionals/${uniCode}/${degreeId}`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleAskCourseTeacher = (data) => {
    ...
    fetch(
      `http://localhost:5000/api/validations/${data.unicodesrc}/${data.degreeidsrc}/${data.courseidsrc}/${data.unicodedst}/${data.degreeiddst}/${data.courseiddst}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleBlockchain = async (
    ...
    const dbStudentsResponse = await fetch(
      `http://localhost:5000/api/validates/validation/${unicodesrc}/${degreeidsrc}/${courseidsrc}/${unicodedst}/${degreeiddst}/${courseiddst}`
    );
      ...
      try {
        ...
        if (res.ok && data.addressid) {
          ...
          const teacherAlreadyAllowed =
            teachersData.teachers.includes(courseDstAddress);
          console.log(
  ...
  // Function to handle the cancelation of a Validation Petition
  const handleCancelPetition = (valid) => {
    ...
    fetch(
      ...
      .then((data) => {
        setRefreshKey((prev) => prev + 1);
        console.log("Successfully updated provisional:", data);
        setFilteredValidations((prevValidations) =>
                    ...
                    {valid.provisional === 2 ? (
                      ...
                      <>
                        ...
                        <td>
                          <button
                            style={buttonStyle}
                            onClick={() => handleCancelPetition(valid)}
                          >
                            Cancel
                          </button>
                        </td>
                        <td>
                          {valid.provisional === 3 ? (
                            ...
                            <button
                              style={buttonStyle}
                              onClick={() => handleAskCourseTeacher(valid)}
                            >
                              Ask Course Teacher
                            </button>
                          )}
