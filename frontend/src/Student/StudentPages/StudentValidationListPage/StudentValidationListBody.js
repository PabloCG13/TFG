...
const StudentValidationListBody = ({ studentId }) => {
  ...
  useEffect(() => {
    //Get the coordinator of the student
      ...
      .then((data) => {
        ...
        if (uniqueUniCodes.length > 0) {
          fetch(`http://localhost:5000/api/universities/exclude`, {
            ...
            .catch((error) =>
              console.error("Error fetching excluded universities:", error)
            );
        }
      })
    ...
    uniqueUnicodes.forEach((unicodedst) => {
      fetch(`http://localhost:5000/api/universities/${unicodedst}`)
        .then((response) => {
          if (!response.ok) {
            throw new Error(
              `Failed to fetch university for ${unicodedst}. Status: ${response.status}`
            );
          }
          return response.json();
        })
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/api/validates/${studentId}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Failed to fetch student validation petitions for ${studentId}. Status: ${response.status}`
          );
        }
        return response.json();
      })
      ...
      .catch((error) =>
        console.error("Error fetching student validation petitions:", error)
      );
    fetch(
      `http://localhost:5000/api/transcripts/erasmusTranscript/${studentId}`
    )
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Failed to fetch erasmus courses in transcript for ${studentId}. Status: ${response.status}`
          );
        }
        return response.json();
      })
      ...
      .catch((error) =>
        console.error(
          "Error fetching student erasmus courses in transcript:",
          error
        )
      );
  ...
  useEffect(() => {
    ...
    studentInfo.forEach(({ unicode, degreeid }) => {
      fetch(
        `http://localhost:5000/api/courses/remaining/${unicode}/${degreeid}/${studentId}`
      )
        .then((response) => response.json())
        .then((data) => setCourses(data))
        .catch((error) =>
          console.error(
            `Error fetching courses for ${unicode}, ${degreeid}:`,
            error
          )
        );
    });
  ...
  const handleNotifyPetition = async (validatid) => {
    ...
    const dbResponse = await fetch(
      `http://localhost:5000/api/validates/${studentId}`
    );

    if (!dbResponse.ok)
      throw new Error(`Failed to fetch studies. Status: ${dbResponse.status}`);
    ...
    const dbResponseValidates = await fetch(
      `http://localhost:5000/api/validates`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    if (!dbResponseValidates.ok) {
      console.error(
        `Failed to add validate entry to DB. Status: ${dbResponseValidates.status}`
      );
      return false;
    }
  ...
  const handleBlockchain = async (
    ...
    const dbDstResponse = await fetch(
      `http://localhost:5000/api/courses/${unicodedst}/${degreeiddst}/${courseiddst}`
    );
    ...
    const dbResponse = await fetch(
      `http://localhost:5000/api/addresses/participant/${teacherDst}`
    );
    ...
    console.log("Studies", studentInfo);
    ...
    if (!degreeResponse.ok)
      throw new Error(`Failed to fetch degree ${degreeId} at ${uniCode}`);
    const degreeData = await degreeResponse.json();

    console.log("DegreeCoordinator ID", degreeData.teacherid);

    const dbResponseCoord = await fetch(
      `http://localhost:5000/api/addresses/participant/${degreeData.teacherid}`
    );
    ...
    // Get srcUni address
    const dbResponseUni = await fetch(
      `http://localhost:5000/api/addresses/participant/${unicodesrc}`
    );
    const dbDataUni = await dbResponseUni.json();

    if (!dbResponseUni.ok || !dbDataUni.addressid) {
      setMessage(
        "No blockchain address found for this user. Please contact support."
      );
      console.error("Database error:", dbDataUni);
      return;
    }

    const uniAddress = dbDataUni.addressid;
    ...
    try {
      ...
      const dbStudentsResponse = await fetch(
        `http://localhost:5000/api/validates/${unicodesrc}/${degreeidsrc}/${courseidsrc}/${unicodedst}/${degreeiddst}/${courseiddst}/${studentId}`
      );
      ...
      dbDataStudents = JSON.stringify({
        ...JSON.parse(validatid),
        studentId: studentId,
        provisional: 1,
      });
    }
      ...
      if (!teacherAlreadyAllowed) {
        try {
          ...
          if (!validationResponse.ok) {
            ...
            console.error(
              `Failed to add validation for student ${student.studentid}. Status: ${validationResponse.status}`,
              errorText
            );
          } else {
            console.log(
              `Validation successfully added for student ${student.studentid}`
            );
          }
        } catch (error) {
          console.error(
  ...
  const changeTranscript = async (
    ...
    try {
      ...
      // Step 3: Update the student's transcript hash
  ...
  const handleChoosePetition = async (validatid) => {
    ...
    handleBlockchain(
      validatid,
      validatid.period,
      validatid.unicodesrc,
      validatid.degreeidsrc,
      validatid.courseidsrc,
      validatid.unicodedst,
      validatid.degreeiddst,
  ...
  const handleSearchComposedValidations = (selCourse, selDegree) => {
    ...
    fetch(
      `http://localhost:5000/api/validations/provisional/composed/${selCourse.unicode}/${selCourse.degreeid}/${selCourse.courseid}/${selDegree.unicode}/${selDegree.degreeid}`
    )
      .then((response) => response.json())
      .then((data) => {
      ...
      .catch((error) =>
        console.error(
          `Error fetching related validations for ${selCourse.courseid}`,
          error
        )
      );
  };
  ...
  const handleSearchInverseValidations = (selCourse, selDegree) => {
    ...
    fetch(
      `http://localhost:5000/api/validations/provisional/inversed/${selCourse.unicode}/${selCourse.degreeid}/${selCourse.courseid}/${selDegree.unicode}/${selDegree.degreeid}`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleRequestValidationComposed = async (validation) => {
    try {
      const dbResponseValidation = await fetch(
      ...
      console.log(
        "Making a new validation with COMPOSED validation: ",
        validation
      );
      console.log("And course: ", selectedCourse);
      if (!dbResponseValidation.ok) {
        throw new Error(
          `Failed to add validation entry to DB. Status: ${dbResponseValidation.status}`
        );
      }
      ...
      if (!dbResponseValidates.ok) {
        throw new Error(
          `Failed to add validation entry to DB. Status: ${dbResponseValidates.status}`
        );
      }
  ...
  const handleRequestValidationInverse = async (validation) => {
    try {
      ...
      console.log(
        "Making a new validation with INVERSE validation: ",
        validation
      );
      console.log("And course: ", selectedCourse);
      if (!dbResponseValidation.ok) {
  ...
  const handleUniversityClick = (uni) => {
    ...
    fetch(`http://localhost:5000/api/universities/${uni.unicode}/degrees`)
      ...
      .catch((error) =>
        console.error(`Error fetching degrees for ${uni.unicode}`, error)
      );
    ...
    <div style={containerStyle}>
      <div style={mainContentStyle}>
                ...
                <tbody>
                  {filteredValidations.map((validatid) => {
                    const hasTakenCourse = studentTranscript.some(
                      (course) =>
                        course.courseid === validatid.courseiddst &&
                        course.erasmus === 1
                    );
                    ...
                    const canChoose =
                      ...
                      (erasmusCredits.length === 0 ||
                        (totalErasmusCredits < 60 &&
                          validatid.unicodedst ===
                            erasmusCredits[0].abroad_university &&
                          validatid.degreeiddst ===
                            erasmusCredits[0].abroad_degree));
                    return (
                          ...
                          studentInfo[0].unicode === validatid.unicodesrc ? (
                            hasTakenCourse ? (
                              ...
                              <button
                                style={buttonStyle}
                                onClick={() =>
                                  openModalForValidation(validatid)
                                }
                              >
                                Choose
                              </button>
        ...
        {activeTab === "RELATED VALIDATIONS" && (
          <div style={tableContainer}>
            <h2 style={tableTitle}>
              Find Related Validations for {studentId}{" "}
            </h2>

            {/* Step 1: Select a course */}
            <div style={{ marginBottom: "20px" }}>
              <label>
                <strong>Select an Origin Course:</strong>
              </label>
              <select
            ...
            <div style={{ marginBottom: "20px" }}>
              <label>
                <strong>Select a Destination University:</strong>
              </label>
              <select
                value={selectedUniversity ? selectedUniversity.unicode : ""}
                onChange={(e) => {
                  const selected = Object.values(universitiesRelated).find(
                    (uni) => uni.unicode === e.target.value
                  );
            ...
            {selectedUniversity && degreesInUni.length > 0 && (
              <div style={{ marginBottom: "20px" }}>
                <label>
                  <strong>Select a Degree:</strong>
                </label>
                {/* Dropdown for role selection */}
                <select
                  onChange={(e) => {
                    const selected = degreesInUni.find(
                      (degree) => degree.degreeid === e.target.value
                    );
                ...
                >
                  ...
                  {degreesInUni.length > 1 ? (
                    ...
                    <option
                      value={selectedDegree ? selectedDegree.degreeid : ""}
                      hidden
                    >
                      Select Degree
                    </option>
            ...
            {selectedCourse && selectedDegree && (
              <div
                style={{ display: "flex", gap: "100px", marginBottom: "10px" }}
              >
                <button
                  onClick={() => {
                    setSearchType("composed");
                    setHasSearched(true);
                    handleSearchComposedValidations(
                      selectedCourse,
                      selectedDegree
                    );
                  }}
                ...
                <button
                  onClick={() => {
                    ...
                    setHasSearched(true);
                    handleSearchInverseValidations(
                      selectedCourse,
                      selectedDegree
                    );
                  }}
            ...
            {hasSearched &&
              ...
              (relatedValidations.length > 0 ? (
                    ...
                    <tbody>
                      {relatedValidations.map((validation, index) => (
                        <tr key={index}>
                          <td style={tdStyle}>
                            {searchType === "inverse"
                              ? validation.courseidsrc
                              : validation.courseiddst}
                          </td>
                          <td style={tdStyle}>{validation.period}</td>
                          <td style={tdStyle}>
                            {validation.provisional === 1 && (
                              <span style={{ color: "green" }}>
                                Definitive validation
                              </span>
                            )}
                            {[0, 2, 3, 4].includes(validation.provisional) && (
                              <span style={{ color: "orange" }}>
                                Pending validation
                              </span>
                            )}
                            {validation.provisional === 5 && (
                              <span style={{ color: "red" }}>
                                WARNING! Rejected validation
                              </span>
                            )}
                          </td>
                          <td style={tdStyle}>
                ...
                <p style={{ marginTop: "20px", color: "red" }}>
                  No related validations found for this course and that
                  destination degree.
                </p>
              ))}
          </div>
        ...
        {isModalOpen && selectedValidation && (
          <div style={modalOverlayStyle} onClick={closeModal}>
            <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
              ...
              >
                ...
                {(() => {
                  ...
                  const maxYear = parseInt(
                    selectedValidation.period.split("-")[1]
                  );
              ...
              <button style={buttonStyle} onClick={handleConfirmValidation}>
                Confirm
              </button>
            </div>
          </div>
