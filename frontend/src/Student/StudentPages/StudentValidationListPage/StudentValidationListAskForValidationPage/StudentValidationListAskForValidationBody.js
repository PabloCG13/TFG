...
const StudentValidationListAskForValidationBody = ({ studentId }) => {
  ...
  const [showSelectedCoursesTable, setShowSelectedCoursesTable] =
    useState(false);
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/api/studies/${studentId}`)
      ...
      .catch((error) =>
        console.error("Error fetching student's studies:", error)
      );
  }, [studentId]);
  ...
  const handleDegreeClick = (degree) => {
    ...
    fetch(
      `http://localhost:5000/api/courses/degree/${degree.unicode}/${degree.degreeid}`
    )
      .then((response) => response.json())
      .then((data) => {
        const processedCourses = data.map((course) => {
      ...
      .catch((error) =>
        console.error(
          `Error fetching courses for ${degree.unicode}, ${degree.degreeid}`,
          error
        )
      );
  };
          ...
          {showTable &&
            studies.map(({ unicode, degreeid }) => (
              <div key={`${unicode}-${degreeid}`}>
                <h3>
                  University: {unicode} Degree: {degreeid}
                </h3>
        ...
        <div style={rightSectionStyle}>
            ...
            <table border="1" style={universityTableStyle}>
              <thead>
                <tr>
                  ...
                  <th style={universityTableHeaderStyle}>
                    University Location
                  </th>
                </tr>
              </thead>
          ...
          {showSelectedDegreeTable && (
            <div style={detailsContainerStyle}>
              ...
              <table border="1" style={universityTableStyle}>
                ...
                <tbody>
                  {degreesInUni.map((degree) => (
                    ...
                    >
                      <td style={universityTableCellStyle}>
                        {degree.unicode},{degree.degreeid}
                      </td>
              ...
              <button style={backButtonStyle} onClick={handleBackUniversity}>
                Back
              </button>
            </div>
          )}
          ...
          {showSelectedCoursesTable && (
            <div style={detailsContainerStyle}>
              ...
              <table border="1" style={universityTableStyle}>
                ...
                <tbody>
                  {coursesInDegree.map((course) => (
                    <tr
                      key={course.courseid}
                      onClick={() => handleDstCourseClick(course)}
                    >
                      <td style={universityTableCellStyle}>
                        {course.courseid}
                      </td>
              ...
              <button style={backButtonStyle} onClick={handleBackDegree}>
                Back
              </button>
            </div>
          )}
              ...
              <table border="1" style={universityTableStyle}>
                ...
                <tbody>
                  <tr>
                    <td style={universityTableCellStyle}>Course ID</td>
                    <td style={universityTableCellStyle}>
                      {selectedDstCourse.courseid}
                    </td>
                  </tr>
                  <tr>
                    <td style={universityTableCellStyle}>Name</td>
                    <td style={universityTableCellStyle}>
                      {selectedDstCourse.name}
                    </td>
                  </tr>
                  <tr>
                    <td style={universityTableCellStyle}>Credits</td>
                    <td style={universityTableCellStyle}>
                      {selectedDstCourse.credits}
                    </td>
                  </tr>
                  <tr>
                    <td style={universityTableCellStyle}>Period</td>
                    <td style={universityTableCellStyle}>
                      {selectedDstCourse.period}
                    </td>
                  </tr>
                </tbody>
              </table>
      ...
      <div style={fixedFooterStyle}>
        {selectedCourse && selectedDstCourse && (
          <div style={acceptButtonContainerStyle}>
            ...
            <button onClick={handlePetition} style={acceptButtonStyle}>
              Accept
            </button>
          </div>
        )}
