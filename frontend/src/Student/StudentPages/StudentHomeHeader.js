    ...
    try {
      const responses = await Promise.all(
        validations.map((valid) =>
          fetch(
            `http://localhost:5000/api/validates/${valid.unicodesrc}/${valid.degreeidsrc}/${valid.courseidsrc}/${valid.unicodedst}/${valid.degreeiddst}/${valid.courseiddst}/${valid.studentid}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
        ...
        <div style={containerStyle}>
          ...
          <Link
            ...
            onClick={(e) => {
              ...
              }, 500); // Delay navigation to ensure update
            }}
            ...
            >
              ...
              {(marks.length > 0 || validations.length > 0) && (
                <span style={notificationBadgeStyle}>
                  {marks.length + validations.length}
                </span>
              )}
            </button>
          </div>
              ...
              <ul>
                {marks.map((mark, index) => (
                  <li key={`mark-${index}`}>
                    {mark.provisional === 0 ? (
                      <>
                        Your grade for <strong>{mark.courseid}</strong> is now
                        provisonal
                      </>
                    ) : (
                      <>
                ...
                {validations.map((val, index) => (
                  <li key={`val-${index}`}>
                    Your validation petition for{" "}
                    <strong>{val.courseidsrc}</strong> has changed to a new
                    state
                    <Link
